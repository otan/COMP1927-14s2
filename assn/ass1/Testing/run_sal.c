...
int main(int argc, char *argv[])
{
   ...
   int  quiet = 1;    // flag to reduce output "noise"
   int  size = MEMORY_SIZE; // default

   setbuf(stdout, NULL); // don't buffer stdout

   // sort out quiet-ness
   if (argc > 2 && argv[2][0] == 'q') quiet = 1;
   if (argc > 1 && isdigit(argv[1][0])) size=atoi(argv[1]);
   if (size < 64) size = 64;
   ...
   // start the allocator
   sal_init(size);
   printf("sal_init(%d) gives memory[] %d bytes @ %p\n", size, msz(), mem());
   sal_stats();
