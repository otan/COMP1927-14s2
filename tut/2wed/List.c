...
ListNode *generateSequence(int n) {
    if (n == 0) {
        return NULL;
    }
    ListNode *newNode = createNode(n);
    newNode->next = generateSequence(n - 1);
    return newNode;
}

void freeListNodes(ListNode *curr) {
    if (curr == NULL) {
        return;
    }
    freeListNodes(curr->next);
    free(curr);
}
